        ...
        if (player.isSneaking())
            ...
            if (!world.isRemote) {
            	...
            	if (TE instanceof TilePortableHouse){
            		TilePortableHouse tile = (TilePortableHouse) world.getBlockTileEntity(x, y, z);
                	
                	if (tile != null) {
                	GuiPortableHouse gui = new GuiPortableHouse(player, world, tile, null);
                	FMLCommonHandler.instance().showGuiScreen(gui);
                	}
            	}
            	else if (TE instanceof TilePortableHouseDeployer){
            		TilePortableHouseDeployer tile = (TilePortableHouseDeployer) world.getBlockTileEntity(x, y, z);
                	
                	if (tile != null) {
                	GuiPortableHouse gui = new GuiPortableHouse(player, world, null,  tile);
                	FMLCommonHandler.instance().showGuiScreen(gui);
                	}
            	}
            }
    ...
    public void onBlockPlacedBy(World world, int i, int j, int k, EntityLiving entity, ItemStack item)
    {
    	...
    	if(item.hasTagCompound() && world.getBlockTileEntity(i, j, k) instanceof TileAdvancedPortableHouseDeployer)
    	{
    		...
    		if (tile != null)
    		{
    			...
    			if (item.getTagCompound().getString("name") != null){tile.name = item.getTagCompound().getString("name");}
    			tile.hasTag = item.getTagCompound().getIntArray("hasTag");
    			tile.tag = item.getTagCompound();
    		}
    	}
    	
    	else if(item.hasTagCompound() && world.getBlockTileEntity(i, j, k) instanceof TileAdvancedPortableHouse)
    	{
    		TileAdvancedPortableHouse tile = (TileAdvancedPortableHouse) world.getBlockTileEntity(i, j, k);
    		
    		if (tile != null)
    		{
    			if (item.getTagCompound().getInteger("size") != 0){tile.size = item.getTagCompound().getInteger("size");}
    			if (item.getTagCompound().getInteger("hight") != 0){tile.hight = item.getTagCompound().getInteger("hight");}
    			if (item.getTagCompound().getString("name") != null){tile.name = item.getTagCompound().getString("name");}
    		}
    	}
    	
    	else if(item.hasTagCompound() && world.getBlockTileEntity(i, j, k) instanceof TilePortableHouseDeployer)
    	{
    		TilePortableHouseDeployer tile = (TilePortableHouseDeployer) world.getBlockTileEntity(i, j, k);
    	...
    	else if(item.hasTagCompound() && world.getBlockTileEntity(i, j, k) instanceof TilePortableHouse)
    	{
    		TilePortableHouse tile = (TilePortableHouse) world.getBlockTileEntity(i, j, k);
