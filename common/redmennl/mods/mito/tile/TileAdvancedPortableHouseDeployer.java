...
public class TileAdvancedPortableHouseDeployer extends
        TilePortableHouseDeployer
{
    ...
    public void deploy()
    {
        ...
        for (int i = 0; i < idArr.length; i++)
        {
            ...
            if (idArr[i] != 0)
            {
                ...
                if (hasTag[i] == 1)
                {
                    TileEntity TE = world.getBlockTileEntity(xStart + xArr[i],
                            yCoord - 1 + yArr[i], zStart + zArr[i]);
                    ...
                    nbt.setInteger("y", yCoord - 1 + yArr[i]);
