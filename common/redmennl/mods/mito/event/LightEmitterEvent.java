...
public class LightEmitterEvent
{
    ...
    public void onPlayerMove(LivingEvent.LivingUpdateEvent e)
    {
        if (e.entityLiving instanceof EntityPlayer)
        {
            ...
            {
                if (isWearingMinersHelmet(p.inventory)
                        || hasSelectedLightEmittingBlock(p.inventory))
                {
                    Pos3 pos = new Pos3(MathHelper.floor_double(p.posX),
                            MathHelper.floor_double(p.posY) + 1,
                            MathHelper.floor_double(p.posZ));

                    if (p.ridingEntity != null)
                    {
                        pos = new Pos3(
                                MathHelper.floor_double(p.ridingEntity.posX),
                    ...
                    {
                        p.worldObj.setBlock(pos.x, pos.y, pos.z,
                                BlockIds.LIGHTEMITTER, 0, 3);
                    }
                    ...
                    if (usernameLastPosMap.containsKey(p.username))
                    {
                        ...
                        if ((pos2.x != pos.x || pos2.y != pos.y || pos2.z != pos.z)
                                && p.worldObj
                                        .getBlockId(pos2.x, pos2.y, pos2.z) == BlockIds.LIGHTEMITTER)
                        {
                            p.worldObj
                                    .setBlock(pos2.x, pos2.y, pos2.z, 0, 0, 3);
                        }
                    }
    ...
    public static boolean isWearingMinersHelmet(InventoryPlayer inv)
    {
        ...
        if (item != null
                && (item.itemID == ItemIds.MINERSIRONHELMETON
                        || item.itemID == ItemIds.MINERSGOLDHELMETON || item.itemID == ItemIds.MINERSDIAMONDHELMETON))
            return true;
    ...
    public static boolean hasSelectedLightEmittingBlock(InventoryPlayer inv)
    {
        ...
        if (item != null
                && (item.itemID == Block.torchWood.blockID
                        || item.itemID == Block.glowStone.blockID
                        || item.itemID == Block.pumpkinLantern.blockID
                        || item.itemID == ItemIds.MINERSIRONHELMETON
                        || item.itemID == ItemIds.MINERSGOLDHELMETON || item.itemID == ItemIds.MINERSDIAMONDHELMETON))
            return true;
